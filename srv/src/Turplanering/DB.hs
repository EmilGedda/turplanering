...
class Monad m => MonadRDBMS m where
    selectTrailFields :: [DBSections] -> m [DBTrail]
    selectSectionsInside :: Box -> m [DBSections]
...
instance MonadIO m => MonadStorage (Handle m) where
    getDetails bbox = do
        sections <- selectSectionsInside bbox
        trailData <- selectTrailFields sections
        return $ Details (buildTrails trailData sections) []


instance MonadIO m => MonadRDBMS (ReaderT Connection m) where
    selectSectionsInside = select . sectionsInside
    selectTrailFields = select . trailsFrom . map trailId


select :: (MonadReader Connection m, MonadIO m, Default FromFields a b) => Select a -> m [b]
